...
def get_rdf(
    ...
    if whether_to_plot:
        ...
        try:
            ...
            title_m = f'{conc:.3f} M {center_res}PF6'
        except Exception:
            title_m = ''
        if title_m:
...
current_dir = "."
folder_list = glob.glob('transport_results*')[:1]
paths = folder_list
paths.sort()
center_res = "LI"
# center_res = "NA"
...
for idx, folder in enumerate(sorted(paths)):
    # print(i)
    pdbf = f"{folder}/solvent_salt.gro"
    trjf = f"{folder}/nvt.dcd"
    print(idx, folder)
    ...
    resnames.remove(center_res)
    resnames.sort()
    for jdx, i in enumerate(resnames):
        resnames[jdx] = i.replace("(","?").replace(")","?").replace("]","?")
    print(resnames)
    # RDF
    get_rdf(u, cation=center_res, resnames=resnames, names=["O F N"]*len(resnames), start=1500, figname=f"{folder}/rdf.png")
    # Li+ solvation shell
